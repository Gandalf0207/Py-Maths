...
def write(doc, num_exo):
   ...
   # Définitions de la taille de police d'écriture
    ...
    with doc.create(Section("Test resolution partie 1 : ")):
        doc.append(NoEscape("\\ $ %s - %sx = %s + %s $" % (nb1, nb2, nb3, nb4)))
        doc.append(Command('vspace', '5mm'))
        doc.append(NewLine())
        doc.append(NoEscape("\\ $ %s - %s - %sx = %s + %s -%s $" % (nb1, nb1, nb2, nb3, nb4, nb1 )))
        doc.append(Command('vspace', '5mm'))
        doc.append(NewLine())
        newnb_1 = nb3 + nb4 + nb1
        doc.append(NoEscape("\\ $ \\frac{%sx}{%s} = \\frac{%s}{%s} $" % (nb2, nb2, newnb_1, nb2)))
        doc.append(Command('vspace', '5mm'))
        doc.append(NewLine())
        result_1 = round((newnb_1 / nb2),1)
        doc.append(NoEscape("\\ $ x = %s $" % (result_1)))
