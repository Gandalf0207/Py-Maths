...
nb13 = random.randint(1,50)
nb14= random.randint(1,50)
...
with doc.create(Section("Section de test", numbering = False)):
    ...
    doc.append(NoEscape("\\begin{cases}"))
    doc.append(NoEscape("\\  %s(%sx - %sy) + %sy &= %sy\\frac{%s}{%s} \\\\" % (nb1, nb2,nb3,nb4, nb5,nb6,nb7)))
    doc.append(NoEscape("\\  %sx - \\frac{%sy + %s}{%s} &= %s(%sx - %s) \\\\" % (nb8,nb9,nb10,nb11,nb12,nb13,nb14)))
    doc.append(NoEscape("\\end{cases}"))
    doc.append(NoEscape("\\\\"))
    ...
    doc.append(NoEscape("\\begin{align*}"))
    doc.append(NoEscape("\\   \\Leftrightarrow %s\\times%sx + %s\\times(-%sy) + %sy &= %sy \\times \\frac{%s}{%s} \\\\" % (nb1, nb2,nb1,nb3,nb4, nb5,nb6,nb7)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_x_1_nv3 = round(nb1*nb2,2)
    new_y_1_nv3 = round(nb1*nb3,2)
    new_nb_1_nv3 = round(nb5*(nb6/nb7),2)

    doc.append(NoEscape("\\  \\Leftrightarrow %sx - %sy + %sy &= %sy \\\\" % (new_x_1_nv3,new_y_1_nv3,nb4,new_nb_1_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_y_2_nv3 = abs(-new_y_1_nv3+nb4)
    doc.append(NoEscape("\\  \\Leftrightarrow %sx - %sy &= %sy\\\\" % (new_x_1_nv3,new_y_2_nv3,new_nb_1_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    doc.append(NoEscape("\\  \\Leftrightarrow %sx - %sy + %sy &= %sy + %sy\\\\" % (new_x_1_nv3,new_y_2_nv3,new_y_2_nv3 ,new_nb_1_nv3, new_y_2_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_y_3_nv3 = round(new_nb_1_nv3+new_y_2_nv3,2)
    doc.append(NoEscape("\\  \\Leftrightarrow %sx &= %sy \\\\" % (new_x_1_nv3,new_y_3_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    doc.append(NoEscape("\\  \\Leftrightarrow \\frac{%sx}{%s} &= \\frac{%sy}{%s} \\\\" % (new_x_1_nv3,new_x_1_nv3,new_y_3_nv3,new_x_1_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_y_4_nv3 = round(new_y_3_nv3/new_x_1_nv3,2)
    doc.append(NoEscape("\\  \\Leftrightarrow x &= %sy \\\\" % (new_y_4_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())
    ...
    doc.append(NoEscape("\\\\"))
    doc.append(NoEscape("\\ \\text{On remplace x par $%sy$ dans l'équation 1}\\\\" % (new_y_4_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    doc.append(NoEscape("\\begin{align*}"))
    doc.append(NoEscape("\\ \\Leftrightarrow  %s \\times %sy - \\frac{%sy + %s}{%s} &= %s((%s \\times %sy) - %s) \\\\" % (nb8,new_y_4_nv3,nb9,nb10,nb11,nb12,nb13,new_y_4_nv3,nb14)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_nb_2_nv3 = round(nb8*new_y_4_nv3,2)
    new_nb_3_nv3 = round(nb13*new_y_4_nv3,2)
    doc.append(NoEscape("\\ \\Leftrightarrow  %sy - \\frac{%sy + %s}{%s} &= %s((%sy) - %s) \\\\" % (new_nb_2_nv3,nb9,nb10,nb11,nb12,new_nb_3_nv3,nb14)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_y_5_nv3 = round(new_nb_2_nv3*nb11,2)
    new_yb_1_nv3 = round(nb12*new_nb_3_nv3,2)
    new_nb_4_nv3 = round(nb12*nb14,2)
    doc.append(NoEscape("\\ \\Leftrightarrow  \\frac{%sy}{%s} - \\frac{%sy + %s}{%s} &= %sy - %s \\\\" % (new_y_5_nv3,nb11,nb9,nb10,nb11,new_yb_1_nv3,new_nb_4_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_y_6_nv3 = round(new_y_5_nv3-nb9,2)
    doc.append(NoEscape("\\ \\Leftrightarrow    \\frac{%sy - %s}{%s} &= %sy - %s \\\\" % (new_y_6_nv3,nb10,nb11,new_yb_1_nv3,new_nb_4_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    doc.append(NoEscape("\\ \\Leftrightarrow    \\frac{%sy - %s}{%s} - %sy &= %sy - %s - %sy\\\\" % (new_y_6_nv3,nb10,nb11,new_yb_1_nv3,new_yb_1_nv3,new_nb_4_nv3,new_yb_1_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_yb_2_nv3 = round(new_yb_1_nv3*nb11,2)
    doc.append(NoEscape("\\ \\Leftrightarrow    \\frac{%sy - %s}{%s} - \\frac{%sy}{%s} &= %sy - %s - \\frac{%sy}{%s}\\\\" % (new_y_6_nv3,nb10,nb11,new_yb_2_nv3,nb11,new_yb_1_nv3,new_nb_4_nv3,new_yb_2_nv3,nb11)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_y_7_nv3 = round(new_y_6_nv3-new_yb_2_nv3,2)
    doc.append(NoEscape("\\ \\Leftrightarrow   \\frac{%sy - %s}{%s}  &= - %s \\\\" % (new_y_7_nv3,nb10,nb11,new_nb_4_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    doc.append(NoEscape("\\ \\Leftrightarrow   \\frac{%sy - %s}{%s}  \\times %s &= - %s \\times %s \\\\" % (new_y_7_nv3,nb10,nb11,nb11,new_nb_4_nv3,nb11)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())


    new_nb_5_nv3 = round(new_nb_4_nv3*nb11,2)
    doc.append(NoEscape("\\ \\Leftrightarrow   %sy - %s + %s &= - %s + %s \\\\" % (new_y_7_nv3,nb10,nb10,new_nb_5_nv3,nb10)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_nb_6_nv3 = round(new_nb_5_nv3-nb10,2)
    doc.append(NoEscape("\\ \\Leftrightarrow   %sy &= - %s \\\\" % (new_y_7_nv3,new_nb_6_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    doc.append(NoEscape("\\ \\Leftrightarrow   \\frac{%sy}{%s} &=  \\frac{-%s}{%s} \\\\" % (new_y_7_nv3,new_y_7_nv3,new_nb_6_nv3,new_y_7_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())

    new_nb_8_nv3 = round(-new_nb_6_nv3/new_y_7_nv3,2)
    doc.append(NoEscape("\\ \\Leftrightarrow   y &\\approx  %s \\\\" % (new_nb_8_nv3)))
    doc.append(Command('vspace', '5mm'))
    doc.append(NewLine())
